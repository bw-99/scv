...
class LogDCN(BaseModel):
    def __init__(self,
                 ...
                 **kwargs):
        super(LogDCN, self).__init__(feature_map,
                                    model_id=model_id,
                                    gpu=gpu,
        ...
        print("LogDCNLogDCNLogDCNLogDCNLogDCN input_dim", input_dim)
        ...
        self.log_tower = nn.ModuleList([CrossNetwork(input_dim=input_dim,
                                ...
                                batch_norm=batch_norm) for _ in range(num_mask_heads)])
        ...
        final_dim = parallel_dnn_hidden_units[-1] + input_dim * self.num_mask_heads
    ...
    def forward(self, inputs):
        ...
        cross_out = torch.cat([
            self.log_tower[i](feature_emb) for i in range(self.num_mask_heads)
        ], dim=-1)
...
class CrossNetwork(nn.Module):
    def __init__(self,
        ...
        self.agg_batch_norm = nn.BatchNorm1d(input_dim)

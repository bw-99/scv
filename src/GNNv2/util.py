...
class SAGEConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.linear_neigh = nn.ModuleList([
            nn.Linear(in_channels, out_channels) for _ in range(in_channels)
        ])
    ...
    def forward(self, x, adj):
        ...
        out_neigh = torch.zeros_like(neigh_features)
        for idx in range(neigh_features.shape[1]):
            out_neigh[idx] = self.linear_neigh[idx](neigh_features[idx])
        # print(out_neigh.shape, out_self.shape)

        out = torch.zeros_like(out_self)
        row, col = torch.where(adj > 0)
        out[row] = out_self[col] + out_neigh[row]
        return out

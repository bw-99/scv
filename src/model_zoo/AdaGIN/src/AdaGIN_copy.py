...
class AdaGIN(BaseModel):
    def __init__(self,
                 ...
                 sparsity_exp=False,
                 **kwargs):
        super(AdaGIN, self).__init__(feature_map,
                                      model_id=model_id,
                                      gpu=gpu,
        ...
        self.sparsity_exp = sparsity_exp
    
    def forward(self, inputs):
        """
        Inputs: [X,y]
        """
        if self.sparsity_exp:
            return self.forward_sparsity_exp(inputs)
        else:
            return self.forward_real(inputs)

    def forward_sparsity_exp(self, inputs):
        """
        Inputs: [X,y]
        """
        ...
        adj_matrix_lst = self.AutoGraph(feature_emb)
...
class AutoGraph_Layer(nn.Module):
    ...
    def forward(self, feature_emb):
        if self.sparsity_exp:
            return self.forward_sparsity_exp(feature_emb)
        else:
            return self.forward_real(feature_emb)

    def forward_real(self, feature_emb):
        h = feature_emb
        h_list = []
        cold_matrix_lst = []
        for i in range(self.gnn_layers):
            cold_matrix = self.build_cold_matrix(h, self.cold_tau)
            cold_matrix_lst.append(cold_matrix)
            if i == 0:
                self.cold_mask_lst.append(cold_matrix.detach().cpu().mean(dim=0).unsqueeze(dim=0))
            if self.mask is not None and i == 0:
                cold_matrix = cold_matrix * self.mask
        ...
        return cold_matrix_lst

    def forward_sparsity_exp(self, feature_emb):
        h = feature_emb
        h_list = []
        for i in range(self.gnn_layers):
            cold_matrix = self.build_cold_matrix(h, self.cold_tau)

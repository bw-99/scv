...
class LogCNv6_v5(BaseModel):
    def __init__(self,
                 ...
                 **kwargs):
        super(LogCNv6_v5, self).__init__(feature_map,
                                    model_id=model_id,
                                    gpu=gpu,
        ...
        print("LogCNv6_v5LogCNv6_v5LogCNv6_v5LogCNv6_v5LogCNv6_v5 input_dim", input_dim)

        self.log_tower = CrossNetwork(input_dim=input_dim,
                                net_dropout=net_dropout,
                                num_log_layers=num_log_layers,
                                log_layer_norm=log_layer_norm,
                                log_batch_norm=log_batch_norm,
                                exp_layer_norm=exp_layer_norm,
                                exp_batch_norm=exp_batch_norm)
...
class CrossNetwork(nn.Module):
    def __init__(self,
        ...
        self.log_layer_norm = nn.ModuleList()
        self.log_batch_norm = nn.ModuleList()
        self.exp_layer_norm = nn.ModuleList()
        self.exp_batch_norm = nn.ModuleList()
        ...
        for i in range(self.num_log_layers):
            ...
            if log_layer_norm:
                self.log_layer_norm.append(nn.LayerNorm(input_dim))
            if log_batch_norm:
                self.log_batch_norm.append(nn.BatchNorm1d(input_dim))
            if exp_layer_norm:
                self.exp_layer_norm.append(nn.LayerNorm(input_dim))
            if exp_batch_norm:
                self.exp_batch_norm.append(nn.BatchNorm1d(input_dim))
    ...
    def forward(self, x):
        ...
        for idx in range(self.num_log_layers):
            ...
            if len(self.log_batch_norm) > idx:
                x_emb = self.log_batch_norm[idx](x_emb)
            if len(self.log_layer_norm) > idx:
                x_emb = self.log_layer_norm[idx](x_emb)
            
            x_emb = torch.exp(x_emb)
            
            if len(self.exp_batch_norm) > idx:
                x_emb = self.exp_batch_norm[idx](x_emb)
            if len(self.exp_layer_norm) > idx:
                x_emb = self.exp_layer_norm[idx](x_emb)

...
class LogCNv6_v8(BaseModel):
    def __init__(self,
                 ...
                 **kwargs):
        super(LogCNv6_v8, self).__init__(feature_map,
                                    model_id=model_id,
                                    gpu=gpu,
        ...
        print("LogCNv6_v8LogCNv6_v8LogCNv6_v8LogCNv6_v8LogCNv6_v8 input_dim", input_dim)
    ...
    def train_step(self, inputs):
        ...
        hard_loss = self.loss_fn(y_pred, y_true, reduction='mean')
        
        sub_loss_lst = [self.loss_fn(logit_lst[:, idx].unsqueeze(dim=-1), y_true, reduction='mean') for idx in range(logit_lst.shape[-1])]
        sub_loss = torch.stack(sub_loss_lst, dim=-1).mean()

        kd_loss_lst = [self.distillation_loss(logit_lst[:, idx].unsqueeze(dim=-1), y_pred) for idx in range(logit_lst.shape[-1])]
        kd_loss_lst = torch.stack(kd_loss_lst, dim=-1)
        # print(kd_loss_lst, logit_lst[:, 0], y_pred)
        soft_loss = kd_loss_lst.sum()

        loss = (hard_loss + sub_loss)* self.alpha + soft_loss* self.beta

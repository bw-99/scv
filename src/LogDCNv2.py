...
class CrossNetwork(nn.Module):
    def __init__(self,
        ...
        for i in range(self.num_mask_blocks):
            self.w.append(nn.ModuleList([nn.Linear(input_dim, input_dim) for _ in range(self.num_log_heads)]))
            self.masker.append(nn.ModuleList([nn.Linear(input_dim, input_dim) for _ in range(self.num_log_heads)]))
            if layer_norm:
        ...
        self.projection_layer = nn.Linear(input_dim * num_log_heads, input_dim)
    ...
    def forward(self, x):
        ...
        for idx in range(self.num_mask_blocks):
            x_lst = []
            for i in range(self.num_log_heads):
...
#             self.masker.append(nn.ModuleList([nn.Linear(input_dim, input_dim) for _ in range(num_log_heads)]))
#             if layer_norm:
#                 self.layer_norm.append(nn.LayerNorm(input_dim))
#             if batch_norm:
#                 self.batch_norm.append(nn.BatchNorm1d(num_heads))
...
#         self.sfc = nn.Linear(input_dim * num_cross_layers, 1)

#     def forward(self, x):
#         x_lst = []
#         for i in range(self.num_cross_layers):
#             mask = self.masker[i](x)
#             tmpx = torch.log(F.relu(x * mask) + 1)
#             tmpx = torch.exp(self.w[i](tmpx))
#             x_lst.append(tmpx)
#         x_lst = torch.cat(x_lst, dim=-1)
#         logit = self.sfc(x_lst)
#         return logit

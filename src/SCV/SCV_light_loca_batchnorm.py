...
class SCV_light_loca_batchnorm(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="SCV_light_loca_batchnorm",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=10,
                 net_dropout=0.1,
                 ...
                 alpha=0.9,
                 **kwargs):
        super(SCV_light_loca_batchnorm, self).__init__(feature_map,
                                       model_id=model_id,
                                       gpu=gpu,
        ...
        print("SCV_light_loca_batchnorm input_dim", input_dim)
    ...
    def evaluate(self, data_generator, metrics=None):
        ...
        with torch.no_grad():
            ...
            plt.hist(y_pred, bins=50, density=True, alpha=0.75, color='blue')
            plt.title("Power Law Distributed Tensor Histogram")
            plt.xlabel("Value")
            plt.ylabel("Density")
            plt.grid(True)
            save_path = "power_law_histogram.png"
            plt.savefig(save_path, dpi=300)
            logging.info(f"Histogram saved at: {save_path}")
...
class LoCaDistillationLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, true_labels):
        ...
        teacher_probs_corrected = teacher_output * self.alpha  + true_labels * (1-self.alpha)

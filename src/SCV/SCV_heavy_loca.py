...
class SCV_heavy_loca(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="SCV_heavy_loca",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=10,
                 net_dropout=0.1,
                 ...
                 **kwargs):
        super(SCV_heavy_loca, self).__init__(feature_map,
                                       model_id=model_id,
                                       gpu=gpu,
        ...
        print("SCV_heavy_loca input_dim", input_dim)
        ...
        self.W3 = nn.Parameter(torch.randn(self.num_tower, embedding_dim, parallel_dnn_hidden_units[-1]))  # Shape: (d1, d2)
    ...
    def forward(self, inputs):
        ...
        mlp_embeddings = mlp_embeddings.unsqueeze(dim=1).expand(mlp_embeddings.shape[0], self.num_tower, mlp_embeddings.shape[1])

        bilinear_term = torch.einsum('bti,tij,btj->bt', graph_embeddings, self.W3, mlp_embeddings)
        y_pred = self.bias + bilinear_term.sum(dim=-1)

        y_pred = self.output_activation(y_pred)
        return_dict = {"y_pred": y_pred.unsqueeze(dim=-1), "logits": bilinear_term}
        return return_dict
    ...
    def compute_loss(self, return_dict, y_true):
        ...
        if self.distill_loss:
            ...
            for _idx in range(logits.shape[1]):
                _logit = logits[:, _idx].unsqueeze(dim=-1)

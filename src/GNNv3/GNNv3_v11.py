...
class CrossNetwork(nn.Module):
    def __init__(self,
        ...
        for i in range(num_hops):
            # if layer_norm:
            self.layer_norm.append(nn.LayerNorm([num_fields, num_fields]))
            if batch_norm:
    ...
    def forward(self, x, edge_pertub_prob=0):
        ...
        for idx in range(self.num_hops):
            ...
            adj_matrix_hop = self.layer_norm[idx](adj_matrix_hop)
            
            # adj_matrix_hop = adj_matrix_hop - adj_matrix_hop.max(dim=1, keepdim=True)[0]
...
class GNNv3_v11(BaseModel):
    def __init__(self,
                 ...
                 **kwargs):
        super(GNNv3_v11, self).__init__(feature_map,
                                       model_id=model_id,
                                       gpu=gpu,
        ...
        print("GNNv3_v11 input_dim", input_dim)

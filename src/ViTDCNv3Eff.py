...
class CachedMaskFunction(Function):
    cache = {}

    @staticmethod
    def forward(ctx, weights, masker, key):
        if key not in CachedMaskFunction.cache:
            mask = masker(weights)
            CachedMaskFunction.cache[key] = mask
            ctx.save_for_backward(weights)
            ctx.masker = masker
            ctx.key = key
        else:
            mask = CachedMaskFunction.cache[key]
            ctx.save_for_backward(weights)
            ctx.masker = masker
            ctx.key = key
        return mask
...
def generate_cached_mask(weights, masker, key):
    return CachedMaskFunction.apply(weights, masker, key)

class ViTDCNv3Eff(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="ViTDCNv3Eff",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=10,
                 ...
                 **kwargs):
        super(ViTDCNv3Eff, self).__init__(feature_map,
                                    model_id=model_id,
                                    gpu=gpu,
    ...
    def forward(self, inputs):
        ...
        for i in range(self.num_deep_cross_layers):
            weight = self.ECN.w[i].weight
            mask = generate_cached_mask(weight, self.masker, f"emask_{i}")
            emask.append(mask)
        lmask = []
        for i in range(self.num_shallow_cross_layers):
            weight = self.LCN.w[i].weight
            mask = generate_cached_mask(weight, self.masker, f"lmask_{i}")
            lmask.append(mask)
        # Use emask and lmask in ECN and LCN
        dlogit = self.ECN(feature_emb, self.cur_step, emask).mean(dim=1)
        slogit = self.LCN(feature_emb, self.cur_step, lmask).mean(dim=1)
    ...
    def train_epoch(self, data_generator):
        ...
        # Clear the mask cache at the beginning of the epoch
        CachedMaskFunction.cache = {}
...
class ExponentialCrossViTNetwork(nn.Module):
    ...
    def forward(self, x, cur_step, mask):
        for i in range(self.num_cross_layers):
            H = self.w[i](x)
            if len(self.batch_norm) > i:
                H = self.batch_norm[i](H)
            if(self.mask_with_bias):
                x = (x * H + self.b[i]) * mask[i] + x
            else:
                x = (x * (H * mask[i]) + self.b[i]) + x
    ...
    def forward(self, x, cur_step, mask):
        ...
        for i in range(self.num_cross_layers):
            ...
            if len(self.batch_norm) > i:
                H = self.batch_norm[i](H)
            if(self.mask_with_bias):
                x = (x0 * H + self.b[i]) * mask[i] + x
            else:
                x = (x0 * (H * mask[i]) + self.b[i]) + x
            if len(self.dropout) > i:
                x = self.dropout[i](x)
        logit = self.sfc(x)
        return logit

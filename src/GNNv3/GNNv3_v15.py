...
class CrossNetwork(nn.Module):
    def __init__(self,
        ...
        self.k_values = torch.tensor([2**i for i in range(num_tower)])
    ...
    def forward(self, x):
        ...
        masked_matrix = torch.zeros_like(adj_matrix_hop, device=adj_matrix_hop.device)
        for i in range(self.num_tower):
            topk_indices = torch.topk(adj_matrix_hop[i], self.k_values[i], dim=1).indices
            masked_matrix[i].scatter_(1, topk_indices, adj_matrix_hop[i].gather(1, topk_indices))

        mask = (masked_matrix != 0).float()
        adj_matrix_hop = self.layer_norm(masked_matrix.transpose(-2, -1)).transpose(-2, -1)
        x_masked = adj_matrix_hop + (1 - mask) * -1e9
        adj_matrix_hop = torch.nn.functional.softmax(x_masked, dim=1) * mask + self.diag_adj
...
class GNNv3_v15(BaseModel):
    def __init__(self,
                 ...
                 **kwargs):
        super(GNNv3_v15, self).__init__(feature_map,
                                       model_id=model_id,
                                       gpu=gpu,
        ...
        print("GNNv3_v15 input_dim", input_dim)

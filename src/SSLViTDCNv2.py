...
class SSLViTDCNv2(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="SSLViTDCNv2",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=10,
                 num_cross_layers=4,
                 ...
                 vit_num_heads=8,
                 vit_num_ssl=2,
                 vit_ssl_loss_weight=1,
                 **kwargs):
        super(SSLViTDCNv2, self).__init__(feature_map,
                                    model_id=model_id,
                                    gpu=gpu,
        ...
        self.vit_ssl_loss_weight = vit_ssl_loss_weight
    ...
    def forward(self, inputs):
        ...
        cross_out, mask_candidates = self.crossnet(feature_emb)
        ...
        return_dict = {"y_pred": y_pred, "mask_candidates": mask_candidates}
        return return_dict
    ...
    def train_step(self, inputs):
        ...
        mask_loss = 0
        for mask_candidate in return_dict["mask_candidates"]:
            target = mask_candidate[0]
            for random in mask_candidate[1:]:
                mask_loss+= F.mse_loss(target, random).mean()

        loss = loss + (mask_loss) * self.vit_ssl_loss_weight

...
class SCV_ada_tf(BaseModel):
    def __init__(self,
                 ...
                 **kwargs):
        super(SCV_ada_tf, self).__init__(feature_map,
                                       model_id=model_id,
                                       gpu=gpu,
        ...
        print("SCV_ada_tf input_dim", input_dim)
        ...
        self.gating_network = nn.Sequential(
            nn.Linear(embedding_dim * self.num_fields, self.num_tower+1),
            nn.Softmax(dim=1)
        )
        ...
        self.head_weight = nn.Parameter(torch.randn(self.num_tower, embedding_dim, 1))
        self.head_bias = nn.Parameter(torch.randn(1, self.num_tower))

        # self.head_idx = nn.Parameter(torch.range(start=0, end=self.num_tower+1).long(), requires_grad=False)
    ...
    def forward(self, inputs):
        ...
        graph_embeddings = self.gnn_tower(feature_emb).view(feature_emb.shape[0], self.num_tower, -1)
        mlp_logits = self.parallel_dnn(feature_emb.view(feature_emb.shape[0], -1)).view(feature_emb.shape[0], 1)

        logits = torch.einsum("bik,ikj->bi", graph_embeddings, self.head_weight) + self.head_bias
        logits = torch.cat([logits, mlp_logits], dim=-1)
        weights = self.gating_network(feature_emb.view(feature_emb.shape[0], -1))

        y_pred = torch.sum(logits * weights, dim=1)
    ...
    def compute_loss(self, return_dict, y_true):
        ...
        if self.distill_loss:
            ...
            for _idx in range(logits.shape[1]):
                _logit = logits[:, _idx]
                _y = self.output_activation(_logit)
                _loss = self.distill_criterion(_y, return_dict["y_pred"].detach(), y_true)
                loss += _loss
        return loss

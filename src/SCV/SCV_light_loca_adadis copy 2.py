...
class CrossNetwork(nn.Module):
    def __init__(self,
        ...
        self.gnn_transform_weight = Param(torch.Tensor(1, num_tower, num_hops, embedding_dim, embedding_dim))
        ...
        for i in range(num_hops):
            if batch_norm:
                if self.dotproduct:
                    self.batch_norm.append(nn.BatchNorm2d(self.num_tower))
                else:
                    self.batch_norm.append(nn.BatchNorm1d(self.num_tower * self.num_fields * self.embedding_dim))
                # dim = self.num_fields * self.embedding_dim if self.dotproduct else self.num_tower * self.num_fields * 2 * self.embedding_dim
        ...
        for idx in range(self.num_hops):
            if self.dotproduct:
                ...
                if len(self.batch_norm) > idx:
                    # x_reshaped = nei_features.view(nei_features.size(0), self.num_tower, -1)  # => (B, T*N*D)
                    X_new = self.batch_norm[idx](X_new)
                    # nei_features = x_reshaped.view(nei_features.size(0), self.num_tower, self.num_fields, -1)

                # nei_features = self.projector[idx](X_new)
                x = x + first_x * X_new
